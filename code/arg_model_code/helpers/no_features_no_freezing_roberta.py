...
class RoBERTaEnhancedNoFeaturesConfig(RobertaConfig):
...
class RoBERTaEnhancedNoFeatures(PreTrainedModel):
    config_class = RoBERTaEnhancedNoFeaturesConfig
    ...
    def __init__(self, config):
        ...
        self.fine_tuning_layers = torch.nn.ModuleList([
            torch.nn.Sequential(
                torch.nn.Linear(roberta_hidden_size, roberta_hidden_size),
                torch.nn.ReLU(),
                torch.nn.Dropout(0.1)
            ) for _ in range(config.num_fine_tuning_layers)
        ])

        self.rnn = torch.nn.GRU(roberta_hidden_size, 768, batch_first=True, num_layers=config.num_rnn_layers, bidirectional=True)
    ...
    def forward(self, input_ids, attention_mask, labels=None):
        ...
        # RNN
        rnn_output, _ = self.rnn(cls_output.unsqueeze(1))
